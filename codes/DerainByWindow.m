...
fprintf('cut image into pieces of shape (%d, %d)\n', row_num, col_num);
B = zeros(size(Input));
...
for i=1: row_num-1
    for j=1:col_num-1
        ...
        [tmpYB, tmpYR, tmpB, tmpR, tmpE, Bappro, Rappro, tmpMark] = DerainBlock(I, averTheta, flag, bias_lambda); 
...
function [YB, YR, B, R, E, Bappro, Rappro, Mark, M1, M2] = DerainBlock(I, averTheta, flag, bias_lambda)
%%             Paramters
  ...
  [Mark, Theta, ~, amount, propotion, M1, M2] = MaskModifyImpro(Mark, Y, 5, T2, 3, averTheta, flag); %¸Ä¶¯ Y -> I
  ...
  if Theta == 180   % no rain streaks situation
     ...
     fprintf('The propotion of rain in this block is: %.4f%%.\n', propotion*100);
      lambda2 = min(0.01, 0.0045*3.65/(propotion*100+bias_lambda)^2);
     ...
     YB = imfilter(Y-YR, K2, 'symmetric', 'conv');
      ...
      % [B, R, E] = derainWithEdge(Y.*255, (Y-YR).*255, YR.*255, 0.72, lambda2, 1.9, 0.5, 0.5, w1, w2, 1e-4);
      % 0.72, lambda2, 1.9, 0.5, 0.5
      % 0.6 0.001 1.5 0.3 0.2
      % 0.8       1.75 0.5 0.6
      % 0.65      1.75 0.3 0.4
      % w1 = 0; w2=0;
      % [B, R] = DerainSPBregman(Y.*255, (Y-YR).*255, YR.*255, 3.5, lambda2, 9.1, w1, w2, 3e-3);
      % [B, R] = DerainSPBregman(Y.*255, (Y-YR).*255, YR.*255, 1.45, lambda2, 0, w1, w2, 3e-3);
      % 3 0.015 10.1
      % 3 lambda2 9.1 w1 w2 3e-3
      % E = zeros(size(Y));
      if fliplr_flag 
          I = fliplr(I);
