...
% [Y, ~, ~] = rgb2yuv(img);
  % YUV = rgb2ycbcr(I);
  % Y = YUV(:, :, 1);
  Mark = RainDetectConv(img, 9, 7, 0.01, 0.5);
  T2 = 25;
  ...
  [MarkL, ThetaL, ~, ~, propotionL] = MaskModifyImpro(MarkL, img(:, 1:halfW+1, :), 5, T2, 3, 0, 0);
  [MarkR, ThetaR, ~, ~, propotionR] = MaskModifyImpro(MarkR, img(:, halfW:end, :), 5, T2, 3, 0, 0);
  ...
  if double(propotionL/propotionR) < 0.3 || double(propotionL/propotionR) > 3
      cols = 2;
      % propotions(1) = propotionL;
      % propotions(2) = propotionR;
      % thetas(1) = ThetaL;
