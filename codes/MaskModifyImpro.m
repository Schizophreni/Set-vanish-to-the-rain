function [Mask, theta, Theta_cluster, amount, proportion, M1, M2] = MaskModifyImpro(Mark, I,  T1, T2, T3, averTheta, flag) % 改动 Y->I
...
proportion = sum(sum(Mask.*Y))/(size(Mask, 1)*size(Mask, 2));
proportionu = sum(sum(Mask.*(abs(U)+Y)))/(size(Mask, 1)*size(Mask, 2));
proportionv = sum(sum(Mask.*(abs(V)+Y)))/(size(Mask, 1)*size(Mask, 2));
fprintf('proportion of rain is: %.4f%%\n', 100*proportion); 
fprintf('proportionu of rain is: %.4f%%\n', 100*proportionu); 
fprintf('proportionv of rain is: %.4f%%\n', 100*proportionv); % title('Final Mark');
% pause;
Mask = 1 - Mask;
t2 = toc;
